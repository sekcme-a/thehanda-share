...
import styles from "../styles/confirmSurvey.module.css"
...
const ConfirmProgram = () => {
  ...
  useEffect(()=>{
    ...
    const fetchData = async () => {
      let fileData = []
      const query = await db.collection("team").doc(teamId).collection("surveys").where("condition","==", "waitingForConfirm").get()
  ...
  const onFileClick = (id) => {
    router.push(`/${teamId}/editSurvey/${id}`)
  }
