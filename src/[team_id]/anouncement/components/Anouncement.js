...
const Anouncement = () => {
 ...
 const onDeleteClick = async () => {
  ...
  await Promise.all(filesChecked.map(async (fileId) => {
    const doc = await db.collection("team").doc(teamId).collection("anouncements").doc(fileId).get();
    if (doc.exists) {
      if (doc.data().team.includes(user.uid) || userData.roles[1] === "super") {
        await db.collection("team").doc(teamId).collection("anouncements").doc(fileId).delete();
      } else {
        noAuthority = true
      }
    }
